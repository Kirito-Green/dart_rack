...
static DartRack_t dart_rack;         // 飞镖架状??
static RC_ctrl_t remote;             // 遥控器状??
static OfflineMonitor_t offline;     // 模块离线状??
static RefereeInformation_t referee; // 裁判系统数据

/* 飞镖状?? */
static bool_t dart_rack_cmd_flag;                  // 客户端命令标??
static bool_t dart_rack_reset_flag;                // 飞镖架复位标??
static int8_t dart_rack_client_cmd;                // 客户端命??
static int8_t dart_rack_aim_shoot;                 // 飞镖架瞄准点
static int8_t dart_rack_num_count;                 // 飞镖架发射次??
static DartRackShootState_e dart_rack_shoot_state; // 飞镖发射状??
static MotorLoadState_e motor_load_state;          // 装填电机状态机
static MotorDragState_e motor_drag_state;          // 拖拽电机状态机
static MotorAdjustState_e motor_adjust_state;      // 调节电机状态机
static ServoLoadState_e servo_load_state;          // 装填舵机状态机
static ServoLaunchState_e servo_launch_state;      // 发射舵机状态机
static ServoBlockState_e servo_block_state;        // 挡板舵机状态机

/* 事件记录 */
static uint32_t dart_rack_shoot_count; // 飞镖架发射计??
...
static uint32_t servo_block_count;     // 挡板舵机计时

static uint32_t motor_drag_max_time; // 可变时间
...
/**
 * @brief 函数“servo_init”在中断模式下启?? TIM4 ?? TIM5 的基本定时器??
 *  TIM4 ?? TIM5用于控制舵机的装填和发射
 */
void servo_init(void)
{
...
/**
 * @brief 函数“servo_start”启动两个不同定时器的多个通道?? PWM 输出??
 *  舵机启动pwm输出
 */
void servo_start(void)
{
...
/**
 * @brief 函数“servo_no_force”停止两个不同定时器的多个通道?? PWM 信号??
 * 舵机关闭pwm输出
 */
void servo_no_force(void)
{
...
/**
 * @brief 函数“servo_load_up”将舵机角度设置为待装填状??
 */
void servo_load_up(void)
{
...
/**
 * 函数“servo_load_down”将舵机角度设置为装填状??
 */
void servo_load_down(void)
{
...
/**
 * 函数“servo_launch_up”将发射舵机角度设置为待发射状??
 */
void servo_launch_up(void)
{
...
/**
 * 函数“servo_launch_down”将发射舵机角度设置为发射状??
 */
void servo_launch_down(void)
{
...
/**
 * 函数“servo_block_up”将挡板舵机角度设置为准备状??
 */
void servo_block_up(void)
{
...
/**
 * 函数“servo_block_mid”将挡板舵机角度设置为斜角阻挡状??
 */
void servo_block_mid(void)
{
...
/**
 * 函数servo_block_down将挡板舵机角度设置为垂直阻挡状??
 */
void servo_block_down(void)
{
...
void dart_rack_state_update(void)
{
    ...
    switch (remote.rc.s[0]) {
            /* 右拨杆打到最上，飞镖架进入比赛模??,该模式下摩擦轮参数已调好 */
        case RC_SW_UP:                                // 电机状态更??
            if (dart_rack.state == DART_RACK_MATCH) { // 不要重复设参??
                return;
            }
            servo_start();
            dart_rack_match_state_reset();
            dart_rack_client_cmd = 0;                   // 客户端默认前哨站
            dart_rack_num_count  = 0;                   // 飞镖发射数量计数置零
            temp_yaw_cmd         = ZERO_POINT;          // YAW轴指令发送记??
            dart_rack.target.yaw = YAW_TOWER_ANGLE;     // 默认瞄准前哨??
            dart_rack_aim_shoot  = DART_RACK_AIM_TOWER; // 瞄准标志
            lazer_on();                                 // 开启激??
            dart_rack.state = DART_RACK_MATCH;

            break;
            /* 右拨杆打到中间，飞镖架进入调试模??,该模式下摩擦轮速度可调?? */
        case RC_SW_MID:
            if (dart_rack.state == DART_RACK_TEST) { // 不要重复设参??
                return;
            }
            ...
            if (get_close(dart_rack.motor_measure.shoot_motor_measure.adjust_dist, dart_rack.target.adjust, ADJUST_POSITION_THRESHOLD) ||
                ...
                switch (dart_rack_aim_shoot) {
                    case DART_RACK_AIM_TOWER:
                        ...
                        if (dart_rack_shoot_state == DART_RACK_SHOOT_READY) { //? 防止已装填干??
 ...
 * 该函数计算走完给定距离所需的最长时间??
 *
 * @param dist 参数“dist”的类型为“fp32”，代表“浮??32位”。它是代表距离测量的变量??
 * @param time_measure 单位距离的持续时间(以毫秒为单位)??
 *
 * @return 计算结果，是一?? int16_t 值??
 */
int16_t dist_cal_max_time(fp32 dist, int16_t time_measure)
{
...
/**
 * 函数“motor_reset_target”重置飞镖架左拖动、右拖动和负载的目标值??
 */
...
/**
 * @brief 函数“get_state”检?? DartRack 状态机的当前状态??
 *
